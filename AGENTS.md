...
This is a small Node.js ES module project for collecting Google Analytics 4 data, summarizing it with Groq, and sending reports to Telegram.

- `index.js`: process entry point, cron schedule, and per-site job orchestration.
- `analytics.js`: GA4 Data API report collection.
- `summarizer.js`: Groq summary generation.
- `telegram.js`: Telegram Bot API delivery.
- `config.js`: reads monitored sites from `GA_SITES`, plus GA4 metrics, dimensions, date range, and startup env checks.
...
Keep secrets and site-specific values in `.env`; avoid hardcoding tokens, chat IDs, service account paths, or property IDs in implementation modules.
